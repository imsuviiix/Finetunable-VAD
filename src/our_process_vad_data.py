...
def process_google_speech_train(data_dir):
    ...
    for o in os.listdir(data_dir):
        if os.path.isdir(os.path.join(data_dir, o)) and o.split("/")[-1] != "_background_noise_":
            google.extend(glob.glob(os.path.join(data_dir, o) + '/*.wav'))
    return google

def split_train_val_test(google_dir, data_dir, file_type, test_size=0.1, val_size=0.1, demo=False, demo_rate=0.01):
    if file_type == 'speech':
        X = process_google_speech_train(google_dir)
        Y = load_data(data_dir)
        X.extend(Y)
    
        if demo:
            logging.info(
                f"For Demonstration, we use {int(len(X)*demo_rate)}/{len(X)} speech data. Make sure to remove --demo flag when you actually train your model!"
                )
            
            X = np.random.choice(X, int(len(X)*demo_rate), replace=False)
...
def generate_variety_noise(data_dir, filename, prefix):
    ...
    curr_dir = data_dir
    silence_path = os.path.join(curr_dir, "_noise_more")
    ...
    for file in files:
        ...
        for i in range(
            ...
            file_name = "{}_{}.wav".format(base_name, i)
...
def main():
    ...
    parser.add_argument("--google_data_root", required=True, default=None, type=str)
    ...
    parser.add_argument('--demo_rate', required=False, default=0.01, type=float)
    ...
    speech_data_folder = os.path.join(speech_data_root)
    google_data_root = args.google_data_root
    data_set = "google_speech_recognition_v2"
    google_data_folder = os.path.join(google_data_root, data_set)
    ...
    split_train_val_test(google_data_folder, speech_data_folder, "speech", args.test_size, args.val_size, args.demo, args.demo_rate)

    logging.info(f"Split background data!")
    split_train_val_test(google_data_folder, background_data_folder, "noise", args.test_size, args.val_size, args.demo, args.demo_rate)
    ...
    skip_num_val, speech_seg_num_val, speech_val = load_list_write_manifest(
        ...
        0.2,
        None,
        duration_stride=args.window_length_in_sec/2,
        duration_max= args.window_length_in_sec,
    )
    skip_num_test, speech_seg_num_test, speech_test = load_list_write_manifest(
        speech_data_folder, out_dir, 'testing_list.txt', 'speech', 0.2,
        None,
        duration_stride=args.window_length_in_sec/2,
        duration_max= args.window_length_in_sec,
    )
    ...
    # if we select to generate more background noise data
     ...
     #   logging.info("Start generating more background noise data")
      #  generate_variety_noise(background_data_folder, 'validation_list.txt', 'noise')
       # generate_variety_noise(background_data_folder, 'training_list.txt', 'noise')
        #generate_variety_noise(background_data_folder, 'testing_list.txt', 'noise')
    ...
    skip_num_val, background_seg_num_val, background_val = load_list_write_manifest(
        background_data_folder, out_dir, 'validation_list.txt', 'noise', 0, None, 
        duration_stride=args.window_length_in_sec, 
        duration_max=args.window_length_in_sec
    )
    skip_num_test, background_seg_num_test, background_test = load_list_write_manifest(
        background_data_folder, out_dir, 'testing_list.txt', 'noise', 0, None, 
        duration_stride=args.window_length_in_sec, 
        duration_max=args.window_length_in_sec
    )
    skip_num_train, background_seg_num_train, background_train = load_list_write_manifest(
        background_data_folder, out_dir, 'training_list.txt', 'noise', 0, None, 
        duration_stride=args.window_length_in_sec, 
        duration_max=args.window_length_in_sec
    )
